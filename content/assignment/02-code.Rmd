...
La explicación de esta parte del código se encuentra en la sección correspondiente de la [práctica 1](/assignment/01-code#librerias).[pacman::p_load]{.sidenote}
...
Vamos a cargar la base de datos **ELSOC_ess_merit2016.Rproc_elsoc**, que generamos durante la práctica 1. Se puede llamar desde el directorio en que se guardó anteriormente dando la ruta completa:

```{r eval=FALSE, message=FALSE, warning=FALSE}
load("ruta-hacia-carpeta-local/ELSOC_ess_merit2016.RData") #Cargar base de datos
```
...
Esta asocación también se puede representar de manera más simple con un gráfico, en este caso de cajas o boxplot mediante la función `plot_grpfrq` de `sjPlot`:[sjPlot::plot_grpfrq]{.sidenote}
...
En su forma simple en R se aplica la función `cor` a la[cor]{.sidenote} base de datos, y la guardamos en un objeto que le damos el nombre M para futuras operaciones:
...
Este es el reporte simple, pero no muy amigable a la vista. Para una versión más amable utilizamos la función tab_corr[sjPlot::tab_corr]{.sidenote}:[NOTA: sjPlot actualizó su librería a fines de Mayo de 2021 (versión 2.8.4); para quienes no hayan actualizado a esta versión, la antigua función para tabla de correlaciones es `sjt.corr`]{.sidenote}
...
Una segunda forma de presentar matrices de correlaciones es de manera gráfica con la librería `corrplot`, cuya función `corrplot.mixed` [corrplot::corrplot.mixed]{.sidenote}se aplica al objeto que generamos con la función `cor` (M):
...
Finalmente, también se puede representar la correlación entre dos variables en un gráfico de nube de puntos o scatterplot:[sjPlot::plot_scatter]{.sidenote}
...
Completar el reporte de progreso correspondiente a esta práctica [aquí](https://forms.gle/BSSXa4xXXdjq2DKL9)

# Archivo de código

El archivo de código R de esta práctica se puede descargar [aquí](https://raw.githubusercontent.com/juancarloscastillo/multivariada/master/static/assignment/practica_2.R)
